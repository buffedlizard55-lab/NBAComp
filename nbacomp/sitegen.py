...
leaderboard; backtest records are always displayed separately. Kalshi lists one winner market per team, so
the home team's market is the priced leg (away bets are its NO side); rest/B2B/road-trip features are fed
from schedule history rather than box scores, so they work from the first game of a season.</p>
...
<p>Final scores are cross-checked ESPN ↔ Basketball-Reference (stats.nba.com is unreachable from our
collectors — documented on the Sources page, not silently substituted); every verification (match/mismatch)
is stored. Source reachability is probed each collection run and published on the Sources page.
Discrepancies raise anomalies, are investigated in the research log, and are never silently resolved.</p>
